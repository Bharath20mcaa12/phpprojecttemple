...
<h4>1. Sahasra Deepalankar Seva</h4>
<h4>2. Ganahoma Seva</h4>
<h4>3. Arjitha Brahmotsavam Seva</h4>
<br></br>

<div class="form-box" style="width: 600px">
  <form method="post" align="center" style="background-color: rgb(103, 207, 185)">

      <label for="exampleFormControlInput1" class="form-label">Select the seva from above seva list  [ex:1,2,or 3] </label>
      <input type="number" name="sevano" class="form-control" id="exampleFormControlInput1" placeholder="--Seva number--" max="3">
      ...
      <label for="exampleFormControlInput1" class="form-label"> No of people  </label>
      <input type="number" name="no" class="form-control" id="exampleFormControlInput1" placeholder="--No of people--">
...
$q =mysqli_query($con, "select * from onlineseva WHERE email = '$email' ");
        $row=mysqli_fetch_array($q);
    
if($row==0){
$sql = mysqli_query($con,"INSERT INTO onlineseva VALUES('$sevano','$name','$email','$noofpeople','$date','$address')");
if($sql)
                    {
                        echo '<script>alert("Data Added Successfully");window.location="home.html"</script>';
                    }
                    else
                    {
    ...
    echo '<script>alert("email already presesnt");window.location="onlineservice.php"</script>';
